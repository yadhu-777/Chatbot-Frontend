...
export default function CourseCards() {
  ...
  const courses = ["BCA Class", "BBA Class", "BCom Class", "MBA Class"];
  useEffect(() => {
  ...
  return (
    <div className="course-container">
      {select6 && <ShowClass />}
      {aadmin && (
        <button onClick={handleClassAdd} className="btn-class">
          Add
        </button>
      )}
      ...
      <div className="course-grid">
        {classDetails?.map((course, index) => (
          <div
            onClick={() => handleClass(course.course)}
            key={index}
          ...
          >
            ...
            <div className="del">
              {aadmin && (
                <button
                  onClick={(e) => {
                    e.stopPropagation(); // prevents click from reaching parent div
                    handleClassAdd(); // or your delete logic
                  }}
